<?php	
	...
	$c = new Conectar();  //video 31 hacer join en base de datos
	$conexion = $c->conexion();
	...
	$sql = "SELECT
			...
			FROM
				...
				t_usuarios AS usuario ON archivos.id_usuario = usuario.id_usuario
						INNER JOIN
				t_categorias AS categorias ON archivos.id_categoria = categorias.id_categoria and archivos.id_usuario = '$idUsuario'";
				...
				*/
				 ...
				 ?>
					<tr>
						...
						<td>
							<?php 
								for ($i=0; $i < count($extrensionesValidas) ; $i++) { 
									if($extrensionesValidas[$i] == $mostrar['tipoArchivo']) {
							?>
